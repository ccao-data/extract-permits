...
def save_xlsx_files(df, max_rows, file_base_name):
    ...
    for sheet_name in ("PIN Errors", "Other Errors"):
        ...
        # Stage 1: Hide columns not in `unhidden_columns`
        t = time.perf_counter()
        header_values = {
        ...
        print(f"[{sheet_name}] Hide columns: {time.perf_counter() - t:.2f}s")

        # Stage 2: Wrap text in all cells
        t = time.perf_counter()
        ...
        print(f"[{sheet_name}] Wrap text: {time.perf_counter() - t:.2f}s")

        # Stage 3: Set uniform row heights
        t = time.perf_counter()
        for r in range(1, ws.max_row + 1):
            ws.row_dimensions[r].height = 15
        print(f"[{sheet_name}] Set row heights: {time.perf_counter() - t:.2f}s")

        # Stage 4: Style hyperlinks
        t = time.perf_counter()
        for row in ws.iter_rows(
            min_row=header_row,
            max_row=ws.max_row,
        ...
        print(f"[{sheet_name}] Style hyperlinks: {time.perf_counter() - t:.2f}s")

        # Stage 5: Hide "Other Errors" sheet
        t = time.perf_counter()
        if sheet_name == "Other Errors":
            ws.sheet_state = "hidden"
        print(f"[{sheet_name}] Hide sheet (if applicable): {time.perf_counter() - t:.2f}s")

        print(f"[{sheet_name}] total: {time.perf_counter() - sheet_start:.2f}s\n")

    print(f"All sheets done in {time.perf_counter() - t0:.2f}s")
